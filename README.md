...
**The mix matters as much as the shapes.** The reverse mistake is just as easy: a
shape you own *few* of, appearing in *many* layouts. Near-square frames are the
classic offender — they fall out of almost any tiling by accident, and if only ~25
of your photos are square, that handful lands on screen several times as often as
an ordinary portrait.

The rule of thumb: **frames of a shape should be roughly proportional to photos of
that shape.** Count how many frames across all layouts fall in each shape bucket,
divide by how many photos you own of it, and aim for a similar number everywhere.
Getting that ratio even is what took the gap between the most- and least-shown
photo in the author's library down to 2.6×.
